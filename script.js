...
// ---------- STATE ----------
let userName = null;
let attemptedKey = 'css_quiz_attempts_v2';
let questions = [];
let current = 0;
let correct = 0;
let wrong = 0;
...
function selectOption(el,item){
  ...
  if(chosen===item.a){ el.classList.add('correct'); correct++; perTopic[item.topic].correct++; playFirework(); }
...
// Firework sound
function playFirework(){ const audio=new Audio('firework.mp3'); audio.play(); }

// Finish quiz
function finishQuiz(){
  stopGlobalTimer(); stopQuestionTimer();
  const total=questions.length; const percent=Math.round((correct/total)*100);
  let message='😔 Better luck next time!'; if(percent>=90) message='🎉 Excellent! You scored '+percent+'%!'; else if(percent>=70) message='🙂 Good! You scored '+percent+'%';
  ...
  window.open(waURL,'_blank'); window.open(mailURL,'_blank');
